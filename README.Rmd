...
Thank you for taking this test as part of the BE Group recruitment process. Please read this content carefully.
If you find any issue with the data or critical question please contact with paulo.c@bioinsight.pt and sandra.r@bioinsight.pt.
...
You can download and install R from [CRAN](https://CRAN.R-project.org) and Rstudio from [Posit website](https://posit.co/download/rstudio-desktop/).

## Objectives
We've prepared two small tests for our recruitment process. 
The idea is that you will build the best approach to the problem and answer to the specific questions they propose.
You will be asked to send us your approach and the main outputs in a short data report by the end of the date agreed by email.
...
## Your presentation
You will prepare and present a 5 minutes talk with your achievements for each test, focusing on the critical aspects of the proposed questions. Add the details you may think will help to understand it. Be clear and concise. 
...
### Challenge 1
Build an interactive map where one can query for each facility its name, year of commissioning and installed capacity (using provided e2p data).
...
- Use a raster elevation model to get elevation data from. We are not providing any DEM raster for this test. For 30m SRTM DEM, the tiles for Portugal can be found [here](https://dwtkns.com/srtm30m/). A 25m resampled SRTM can be found [here](https://www.fc.up.pt/pessoas/jagoncal/dems/).

- Remember that some questions will be easier to approach using data.table package (specially directional roll joins).

- You may find it useful to programmatically download and read this 25m SRTM raster tiles directly from [this temporary source](https://drive.google.com/file/d/1D8QNshjsY7X700X8MfXFqO3RoXpKGQ2x/view?usp=sharing).
...
- If running your analysis from Windows OS, you can import data directly from github repo with:
