...
# BE Group recruitment test

Thank you for taking this test as part of the BE Group recruitment
process. Please read this content carefully. If you find any issue with
the data or critical question please contact with
<paulo.c@bioinsight.pt> and <sandra.r@bioinsight.pt>.
...
for you. You can download and install R from
[CRAN](https://CRAN.R-project.org) and Rstudio from [Posit
...
- Map the wind energy facilities in mainland Portugal regarding the
  country and district borders. Apply the appropriate legend and choose
  one aggregation level (legend) to wind energy facilities.
...
year of commissioning and installed capacity (using provided e2p data).
...
| adm1_pt.rda | District boundaries ( level 1 )                                |
...
Individual movements are identified by a single unique ID \[track_id\].
...
- Remember to deal with NA values when summarizing.

- Use a raster elevation model to get elevation data from. We are not
  providing any DEM raster for this test. For 30m SRTM DEM, the tiles
  for Portugal can be found [here](https://dwtkns.com/srtm30m/). A 25m
  resampled SRTM can be found
  [here](https://www.fc.up.pt/pessoas/jagoncal/dems/).

- Remember that some questions will be easier to approach using
  data.table package (specially directional roll joins).

- You may find it useful to programmatically download and read this 25m
  SRTM raster tiles directly from [this temporary
  source](https://drive.google.com/file/d/1D8QNshjsY7X700X8MfXFqO3RoXpKGQ2x/view?usp=sharing).
...
# https://drive.google.com/file/d/1D8QNshjsY7X700X8MfXFqO3RoXpKGQ2x/view?usp=sharing
...
dl <- drive_download(
  as_id("1D8QNshjsY7X700X8MfXFqO3RoXpKGQ2x"), path = temp, overwrite = TRUE)
...
- If running your analysis from Windows OS, you can import data directly
